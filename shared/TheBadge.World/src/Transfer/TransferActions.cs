...
namespace TheBadge.World
{
    ...
    public static class TransferActions
    {
                                  ...
                                  TransferBalance tb, ulong saveSeed)
        {
            ...
            exec.RegisterHandler("transfer.sign_free_agent", new SerbestHandler());
        ...
        sealed class TeklifKurali : IActionRule
        {
            public RejectionReason Check(GameState st, CommandEnvelope env, ActionDef a, IPayloadView p, out string detail)
            {
                ...
                if (BosYuva(st) < 0) { detail = "teklif yuvası dolu"; return RejectionReason.StateConflict; }
        ...
        sealed class CevapKurali : IActionRule
        {
            public RejectionReason Check(GameState st, CommandEnvelope env, ActionDef a, IPayloadView p, out string detail)
            {
                ...
                if (benTeklifEden != t.SiraTeklifEdende)
                { detail = "sıra sende değil"; return RejectionReason.StateConflict; }
                if (SureDoldu(st, t)) { detail = "teklif süresi doldu"; return RejectionReason.StateConflict; }
                return RejectionReason.None;
            }
        }
        ...
        sealed class SerbestHandler : IActionHandler
        {
            public RejectionReason Apply(GameState st, WorldJournal j, CommandEnvelope env, ActionDef a, IPayloadView p, out string detail)
            {
                ...
                j.OyuncuSet(i, PlayerField.SozlesmeKalanHafta, yil * 52);
        ...
        /// <summary>Sıradaki teklif kimliği — AÇIK yuvaların en büyüğünden bir fazlası.
        /// `Guid` ya da zaman tabanlı kimlik YASAK (determinizm).</summary>
        internal static int SonrakiTeklifId(GameState st)
        {
            int max = 0;
            for (int i = 0; i < st.Club.TransferTeklifleri.Length; i++)
                if (st.Club.TransferTeklifleri[i].TeklifId > max) max = st.Club.TransferTeklifleri[i].TeklifId;
            return max + 1;
        }
