...
| Nerede bıraktı? | `session_end` + son ekran/faz |
...
3. Telemetri **oynanışı etkilemez**: yazma hatası oyunu düşürmez, sessizce yutulur ve
   `session_end`e bir bayrak düşer.
...
- **Uygulama maç ortasında kapatılınca dosyada o ana kadarki satırlar DURUYOR** (flush davranışı
  elle denenip raporlanır — playtest'te bu gerçekten olur).
